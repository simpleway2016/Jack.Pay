...
namespace Jack.Pay
{
    class Helper
    {
        ...
        static Helper()
        {
            var folder = Helper.GetSaveFilePath();
            //定期删除临时文件
            DeleteTempFile(folder);
        }

        static void DeleteTempFile(string folder)
        {
            new Thread(()=> {
                while(true)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var writeTime = new FileInfo(filepath).CreationTime;
                            //超过8分钟的临时文件，删除掉
                            if ((DateTime.Now - writeTime).TotalMinutes > 8)
                            {
                                try
                                {
        ...
        public static string GetApplicationPath()
        {
            ...
            try
            {
                ...
                {
                    appFolder = System.Web.HttpRuntime.BinDirectory;
                }
                else
                {
        ...
        /// <returns></returns>
        public static string ReadContentFromResourceStream(string resourcePath)
        {
            var stream = typeof(Helper).Assembly.GetManifestResourceStream(resourcePath);
        ...
        public static string GetSaveFilePath()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(System.Web.HttpRuntime.BinDirectory))
                {
                    ...
                    path = bin + "\\App_Data\\Jack.Pay.Temp";
                }
                else
                {
                    path = Way.Lib.PlatformHelper.GetAppDirectory() + "Jack.Pay.Temp";
                }
            }
            catch
            {
